...
sections_text = claude_haiku(SECTIONS_SYSTEM, sections_prompt, max_tokens=1600)
...
Rules:
— Exactly 3 sentences — not 2, not 4, not 5. Count them before returning.
— Each sentence must be completeable within the token budget — never start a sentence you cannot finish. If you are running long, cut the third sentence short rather than truncating mid-word.
...
— CRITICAL FORMAT RULE: return 3 complete sentences of plain text only — no label, no heading, no bold text, no asterisks, no "BROKER'S LENS" prefix, no markdown of any kind — just three sentences of raw prose ending with a full stop"""
...
brokers_lens = claude_haiku(LENS_SYSTEM, lens_prompt, max_tokens=600)
print(f"  Done. {len(brokers_lens)} chars.")


# ── CLEAN BROKER'S LENS BEFORE ASSEMBLY ──────────────────────────────────────
# Strip any markdown the model added despite instructions, and remove any
# label prefix the model may have included.
import re as _re
brokers_lens = brokers_lens.strip()
# Remove bold markers
brokers_lens = brokers_lens.replace("**", "")
# Remove any leading label the model added (e.g. "Broker's Lens\n", "BROKER'S LENS\n")
brokers_lens = _re.sub(r"(?i)^broker'?s?\s+lens[\s:–—\-]*\n?", "", brokers_lens).strip()
# Check for truncation — if the lens doesn't end with sentence-ending punctuation,
# the model ran out of tokens mid-sentence
lens_truncated = len(brokers_lens) > 0 and brokers_lens[-1] not in ".!?"
...
# Check for header leak — after stripping, any remaining label is a prompt failure
lens_header_leak = bool(_re.search(r"(?i)broker'?s?\s+lens", brokers_lens)) or "**" in brokers_lens
...
if lens_truncated:      flags.append("BROKER'S LENS TRUNCATED — hit token limit mid-sentence, check output")
